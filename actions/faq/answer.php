	...
	if($faq->getSubtype() == "faq"){
		...
		if($addFAQ == "yes"){
			...
			if(!empty($cat)){
				...
				if(remove_metadata($faq->guid, "userQuestion", true)){
					if($faq->save()){
						$url = $CONFIG->wwwroot . "pg/faq/?categoryId=" . get_metastring_id($faq->category) . "#qID" . $faq->guid;
						if($same){
						...
						if(in_array(true, $result)){
							system_message(elgg_echo("faq:answer:succes:added:send"));
						} else {
			...
			if(in_array(true, $result)){
				system_message(elgg_echo("faq:answer:succes:not_added:send"));
			} else {
